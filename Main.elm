...
url : String -> String
url action =
    "http://localhost:9000/member/" ++ action
...
start : ( Model, Cmd Msg )
start =
    ( Model 0 "No message" (Member 1 "Kurt" "Wonn")
    , Cmd.none
    )
...
update msg model =
    case msg of
        ...
        ChangeId idText ->
            --one way to modify only one field of member is to do this
            case String.toInt idText of
                ...
                Err _ ->
                    -- also possible this way where we construct a brand new member with Member constructor
